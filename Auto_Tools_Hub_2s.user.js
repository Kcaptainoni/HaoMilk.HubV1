...
// @version      1.5.0-2s
// @description  2s: tab Tim link = Google "<tu khoa> <domain>". Continue giu nguyen. AI chi khi bam nut.
// @author       You
// @match        *://*/*
// @grant        GM_setValue
// @grant        GM_getValue
...
<div class="as2s-body" id="as2s-tab-search">
  <div style="font-size:11px;color:rgba(255,255,255,.5);margin-bottom:10px;line-height:1.4">
    Đọc <b>từ khóa + domain</b> → Google: <b>từ khóa domain</b> (vd: hút bể phốt moitruongxanh.com). Bạn tự vào. AI chỉ khi bấm nút.
  </div>
  <label>Từ khóa</label>
...
<div class="as2s-body" id="as2s-tab-cfg" style="display:none">
  ...
  <div class="as2s-status" id="as2s-cfg-status"></div>
        ...
        document.getElementById('as2s-scrape').onclick = () => {
            ...
            if (r.domain && document.getElementById('as2s-domain')) {
                document.getElementById('as2s-domain').value = r.domain;
                GM_setValue(STORE.domain, r.domain);
            }
            setSt(r.keyword
                ? ('Từ khóa: ' + r.keyword + (r.domain ? ' | Domain: ' + r.domain : ' | Domain: nhập hoặc bấm AI'))
