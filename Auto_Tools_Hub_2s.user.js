...
(function () {
    ...
    function openGoogleSearch(keyword, domainHint, titleHint, newTab) {
        ...
        // Search chỉ ra đúng domain: site:domain.com từ khóa
        let q = keyword;
        if (domainHint) {
            // Bỏ wildcard *** cho site:
            const clean = domainHint.replace(/\*+/g, '').replace(/^\.+|\.+$/g, '');
            if (clean && clean.includes('.')) {
                q = keyword + ' site:' + clean;
            } else if (clean) {
                q = keyword + ' ' + clean;
            }
        }
