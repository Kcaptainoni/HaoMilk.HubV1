...
// @version      1.4.1-2s
// @description  2s: keyword+domain → Google site:domain (user tự vào). AI chỉ khi bấm nút.
// @author       You
// @match        *://*/*
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    function openGoogleSearch(keyword, domainHint, titleHint, newTab) {
        if (!keyword && !domainHint) return alert('Chưa có từ khóa / domain');
        keyword = String(keyword || '').trim();
        domainHint = String(domainHint || '').trim().replace(/^www\./i, '').replace(/\/+$/, '');
        // Làm sạch domain (bỏ *** và ký tự lạ)
        let cleanDomain = domainHint.replace(/\*+/g, '').replace(/[^a-z0-9.-]/gi, '');
        if (cleanDomain && !cleanDomain.includes('.') && domainHint.includes('.')) {
            cleanDomain = domainHint.replace(/\*+/g, '').trim();
        }

        // Chiến lược search (tránh "không tìm thấy" khi site: + cụm dài):
        // 1) Có domain → ưu tiên: site:domain  (đúng web đó, user tự chọn bài)
        // 2) Thêm vài từ khóa ngắn (không nhét cả câu dài)
        let q = '';
        if (cleanDomain && /\.[a-z]{2,}/i.test(cleanDomain)) {
            q = 'site:' + cleanDomain;
            if (keyword) {
                // Lấy tối đa 3–4 từ đầu của keyword để vẫn lọc bài liên quan
                const words = keyword.split(/\s+/).filter(Boolean).slice(0, 4).join(' ');
                q = q + ' ' + words;
            }
        } else if (keyword) {
            q = keyword + (cleanDomain ? (' ' + cleanDomain) : '');
        } else {
            q = cleanDomain || keyword;
        }

        const url = 'https://www.google.com/search?q=' + encodeURIComponent(q) + '&hl=vi';
        GM_setValue(STORE.keyword, keyword);
        GM_setValue(STORE.domain, cleanDomain || domainHint || '');
    ...
    // Không tự click kết quả Google — user tự chọn / vào web
    ...
    async function runFullFlow(newTab) {
        ...
        let domain = (document.getElementById('as2s-domain') && document.getElementById('as2s-domain').value) ||
            GM_getValue(STORE.domain, '') || '';
        domain = String(domain).trim();
        if (!domain) {
            return alert('Chưa có domain. Bấm nút AI đọc domain (hoặc nhập tay) rồi chạy lại.');
        }
        ...
        if (field) {
            ...
            GM_setValue(STORE.mission, 'website');
        }

        setSt('Google: ' + keyword + ' site:' + domain + ' — bạn tự vào web');
    ...
    async function runSite2sFullFlow(newTab) {
        ...
        if (!keyword) return alert('Chưa có từ khóa (bấm Đọc từ khóa hoặc nhập tay)');

        // Domain chỉ lấy từ ô input — KHÔNG tự gọi AI (tốn token)
        let domain = (document.getElementById('as2s-domain') && document.getElementById('as2s-domain').value) ||
            GM_getValue(STORE.domain, '') || '';
        domain = String(domain).trim();
        if (!domain) {
            return alert('Chưa có domain.\nBấm "AI đọc domain + title" một lần (hoặc nhập tay domain), rồi chạy Full flow.');
        }

        const title = (document.getElementById('as2s-title') && document.getElementById('as2s-title').value) ||
            GM_getValue(STORE.title, '') || '';

        GM_setValue(STORE.s2sEnabled, true);
        GM_setValue(STORE.s2sStep, 'search');
        GM_setValue(STORE.mission, 'site2s_button');
        GM_setValue(STORE.flowArmed, false); // không auto vào
        updateS2sSwitchUI();
        startSite2sWatcher();

        setSt('Google site:' + domain + ' — bạn tự vào web, cuộn tới SITE2S');
        openGoogleSearch(keyword, domain, title, !!newTab);
    }
...
<div class="as2s-body" id="as2s-tab-search">
  <div style="font-size:11px;color:rgba(255,255,255,.5);margin-bottom:10px;line-height:1.4">
    Từ khóa + domain → Google <b>site:domain</b> (bạn tự vào web). AI chỉ khi bấm nút.
  </div>
  <label>Từ khóa</label>
