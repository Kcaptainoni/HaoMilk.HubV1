...
(function () {
    ...
    // layma: từ khóa #TK1/#TK2/#TK3 — domain để AI đọc #hinh_nv
    function scrapeKeywordDomainFromPage() {
        let keyword = '';
        let domain = '';
        try {
            for (const id of ['TK1', 'TK2', 'TK3']) {
                const el = document.getElementById(id) || document.querySelector('#' + id + ', input#' + id + ', [name="' + id + '"]');
                if (!el) continue;
                const v = (el.value || el.textContent || el.innerText || '').trim();
                if (v && v.length >= 2) { keyword = v; break; }
            }
        } catch (e) {}
