...
(function () {
    ...
    // layma: ĐÚNG ô <p id="TK1" class="box-copy-code">…</p>
    function scrapeKeywordDomainFromPage() {
        let keyword = '';
        let domain = '';
        try {
            // Ưu tiên đúng selector user gửi
            ...
            if (el) {
                keyword = (el.textContent || el.innerText || '').replace(/\s+/g, ' ').trim();
            }
            if (!keyword) {
                for (const id of ['TK2', 'TK3']) {
                    const e2 = document.getElementById(id);
                    if (!e2) continue;
                    const v = (e2.textContent || e2.innerText || e2.value || '').trim();
...
(function(){
  ...
  setTimeout(function typeNext(){
    ...
    function findLayMaButton() {
        // layma: vẫn dùng logic nút LẤY MÃ / what-on như link4m (trang đích giống nhau)
    ...
    function ensureUI() {
        ...
        if (!document.getElementById('as-style')) {
            ...
            style.textContent = `
                ...
                #as-btn:active { transform: scale(0.93) !important; }
                #as-panel {
                    position: fixed !important;
                    left: 10px !important;
                    right: 10px !important;
                    bottom: max(72px, calc(56px + env(safe-area-inset-bottom))) !important;
                ...
                .as-switch.on { background: #6366f1 !important; }
                ...
                .as-btn-green { background: linear-gradient(135deg, #22c55e, #16a34a) !important; }
                .as-btn-orange { background: linear-gradient(135deg, #f97316, #ea580c) !important; }
                .back-btn {
