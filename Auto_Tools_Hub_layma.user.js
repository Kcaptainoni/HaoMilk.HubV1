// ==UserScript==
// @name         Auto Tools Hub — layma.net
// @namespace    http://tampermonkey.net/
// @version      6.81-layma
// @description  layma.net standalone: TK1, hinh_nv AI, what-on, dán mã
// @author       You
// @match        *://*/*
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    // Trích mã: link4m (Mã KM), layma (Mã Code #message), gtrafic (trade-btn)
    function extractPromoCode() {
        // layma
        try {
            for (const msg of document.querySelectorAll('#message, [id="message"]')) {
                ...
                if (m && /[A-Za-z]/.test(m[1]) && m[1].length >= 4) return m[1];
            }
        } catch (e) {}
        // gtrafic
        try {
            for (const el of document.querySelectorAll('.trade-btn-clf__content.copy-allowed')) {
                ...
                if (/^[A-Za-z0-9]{4,16}$/.test(t) && /[A-Za-z]/.test(t)) return t;
            }
    ...
    function isGtraficHost() {
        return /gtraffic\.|gtrafic\./i.test(location.hostname || '');
    }
    function isLaymaHost() {
        return /layma\.net/i.test(location.hostname || '');
    }
    function isOriginFormHost() {
        return isLink4mHost() || isGtraficHost() || isLaymaHost();
    }

    function markFlowOrigin() {
        try {
            GM_setValue('as_original_url', location.href);
            GM_setValue('as_flow_origin', 'layma');
            if (isLaymaHost()) {
                GM_setValue('as_layma_home', location.href);
                GM_setValue('as_layma_home_time', Date.now());
            }
        } catch (e) {}
    }
    try {
        if (isOriginFormHost()) markFlowOrigin();
    } catch (e) {}
    ...
    // ---- AI TÁCH RIÊNG: layma (chỉ #TK1 + #hinh_nv) ----
    async function aiFillLayma(autoSearch) {
        const statusEl = document.getElementById('as-lm-ai-status');
        const setSt = (t) => { if (statusEl) statusEl.textContent = t; };
        let keyword = '';
        try {
            const tk = document.getElementById('TK1') || document.querySelector('#TK1, input#TK1, [name="TK1"]');
            if (tk) keyword = (tk.value || tk.textContent || '').trim();
        } catch (e) {}
        try {
            const kEl = document.getElementById('as-lm-keyword');
            if (kEl && keyword) kEl.value = keyword;
        } catch (e) {}
        setSt(keyword ? ('KW: ' + keyword.slice(0, 50)) : 'Không thấy #TK1…');

        let domain = '';
        let img = document.getElementById('hinh_nv');
        if (!img) {
            for (const i of document.querySelectorAll('img')) {
                const src = (i.currentSrc || i.src || '').toLowerCase();
                if (/api\.layma\.net\/media|layma\.net.*posts/i.test(src)) { img = i; break; }
            }
        }
        if (img) {
            setSt('AI đọc domain #hinh_nv…');
            try {
                const inline = await imgToInlineData(img);
                const raw = await callPatewayVision([inline],
                    'Đọc domain/website trong ảnh layma #hinh_nv (có thể bị che). Chỉ lấy chữ trước dấu *.\nTrả 1 dòng:\ndomain: ...');
                const parsed = parseAiKeywordDomain(raw);
                domain = parsed.domain || '';
            } catch (e) {
                setSt('AI lỗi: ' + (e.message || e));
            }
        }
        try {
            const dEl = document.getElementById('as-lm-domain');
            if (dEl && domain) dEl.value = domain;
        } catch (e) {}
        setSt((keyword || '?') + ' | domain: ' + (domain || '(trống)'));
        if (autoSearch && keyword) startSmartSearch(keyword, domain, false);
    }

    // ---- AI TÁCH RIÊNG: gtrafic ----
    async function aiFillGtrafic(autoSearch) {
        const statusEl = document.getElementById('as-gt-ai-status');
        const setSt = (t) => { if (statusEl) statusEl.textContent = t; };
        let keyword = '';
        try {
            const body = (document.body && document.body.innerText) || '';
            const m = body.match(/từ\s*khóa\s*[:：]?\s*([^\n]{2,60})/i);
        ...
        setSt(keyword ? ('KW: ' + keyword.slice(0, 50)) : 'Không thấy từ khóa…');

        let domain = '';
        let img = null;
        for (const i of document.querySelectorAll('img')) {
            const src = (i.currentSrc || i.src || '').toLowerCase();
            if (/cdn\.gtraffic\.io|gtraffic\.io\/image/i.test(src)) { img = i; break; }
        }
        if (img) {
            setSt('AI đọc domain gtrafic…');
            try {
                const inline = await imgToInlineData(img);
                const raw = await callPatewayVision([inline],
                    'Đọc domain trong ảnh. Chỉ lấy chữ trước dấu *.\nTrả 1 dòng:\ndomain: ...');
                domain = parseAiKeywordDomain(raw).domain || '';
            } catch (e) {
                setSt('AI lỗi: ' + (e.message || e));
            }
        }
        try {
            const dEl = document.getElementById('as-gt-domain');
            if (dEl && domain) dEl.value = domain;
        } catch (e) {}
        setSt((keyword || '?') + ' | domain: ' + (domain || '(trống)'));
        if (autoSearch && keyword) startSmartSearch(keyword, domain, true);
    }
    ...
    function startSmartSearch(keyword, domain, newTab) {
        ...
        // gtrafic: ưu tiên tab mới để giữ form
        if (typeof newTab === 'undefined') newTab = isGtraficHost();
        if (newTab) {
            try {
            ...
            if (!w) {
                ...
                try { GM_deleteValue('as_keep_origin_tab'); } catch (e) {}
            } else {
                try { w.focus(); } catch (e) {}
                console.log('[Auto Tools] Đã mở Google tab mới — tab form vẫn giữ');
            }
        } else {
            try {
    ...
    function ensureUI() {
        ...
        if (!document.getElementById('as-style')) {
            ...
            style.textContent = `
                ...
                #as-btn:active { transform: scale(0.94) !important; }
                #as-panel {
                    position: fixed !important;
        ...
        panel.innerHTML = `
            ...
            <div id="panel-gtrafic" style="display:none">
                <div class="as-header">
                    <span>gtrafic</span>
                    <span style="cursor:pointer;opacity:.7;font-size:17px" id="as-close-gt">✕</span>
                </div>
                <div class="as-body">
                    <button class="back-btn" id="btn-back-gt">← Menu chính</button>
                    <div style="font-size:12px;color:#aaa">Từ khóa</div>
                    <input id="as-gt-keyword" placeholder="Từ khóa gtrafic...">
                    <div style="font-size:12px;color:#aaa">Domain</div>
                    <input id="as-gt-domain" placeholder="domain.com">
                    <button class="as-btn-main as-btn-green" id="as-gt-ai">🤖 AI gtrafic (tách riêng)</button>
                    <button class="as-btn-main" id="as-gt-search">Tìm Google (tab mới)</button>
                    <div id="as-gt-ai-status" style="font-size:11px;color:#94a3b8;margin-top:6px;min-height:16px"></div>
                    <div style="font-size:11px;color:#aaa;margin-top:8px;line-height:1.35">AI chỉ điền ô gtrafic — không đụng link4m / layma.</div>
                </div>
            </div>
            ...
            <div id="panel-layma" style="display:none">
                <div class="as-header">
                    <span>layma.net</span>
                    <span style="cursor:pointer;opacity:.7;font-size:17px" id="as-close-lm">✕</span>
                </div>
                <div class="as-body">
                    <button class="back-btn" id="btn-back-lm">✕ Đóng</button>
                    <div style="font-size:12px;color:#aaa">Từ khóa (#TK1)</div>
                    <input id="as-lm-keyword" placeholder="Từ #TK1...">
                    <div style="font-size:12px;color:#aaa">Domain (#hinh_nv)</div>
                    <input id="as-lm-domain" placeholder="domain từ ảnh">
                    <button class="as-btn-main as-btn-green" id="as-lm-ai">🤖 AI layma (tách riêng)</button>
                    <button class="as-btn-main" id="as-lm-search">Tìm Google (cùng tab)</button>
                    <div id="as-lm-ai-status" style="font-size:11px;color:#94a3b8;margin-top:6px;min-height:16px"></div>
                    <div style="font-size:11px;color:#aaa;margin-top:8px;line-height:1.35">
                        AI chỉ #TK1 + #hinh_nv. Chạm màn hình what-on: nên tự chạm (trình duyệt chặn click giả isTrusted).
                    </div>
                </div>
            </div>
        ...
        btn.onclick = (e) => {
            ...
            if (open) {
                try {
                    ['panel-main','panel-gtrafic','panel-link4m'].forEach(id => {
                        const el = document.getElementById(id); if (el) el.style.display = 'none';
                    });
                    const pl = document.getElementById('panel-layma');
        ...
        const close2 = document.getElementById('as-close2');
        if (close2) close2.onclick = () => { panel.style.display = 'none'; };
        const closeLm = document.getElementById('as-close-lm');
        if (closeLm) closeLm.onclick = () => { panel.style.display = 'none'; };

        function showLaymaPanel() {
            try {
                ['panel-main','panel-gtrafic','panel-link4m'].forEach(id => {
                    const el = document.getElementById(id); if (el) el.style.display = 'none';
                });
                const pl = document.getElementById('panel-layma');
                if (pl) { pl.style.display = 'block'; pl.style.setProperty('display','block','important'); }
                panel.style.display = 'block';
            } catch (err) { console.log('[Auto Tools] showLaymaPanel', err); }
        }
        // Nút phụ (nếu còn trong DOM)
        ['btn-open-lm','btn-layma'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.onclick = (e) => { try { e.preventDefault(); e.stopPropagation(); } catch (e2) {} showLaymaPanel(); };
        });
        const backLm = document.getElementById('btn-back-lm');
        if (backLm) {
            backLm.textContent = '✕ Đóng';
            backLm.onclick = () => { panel.style.display = 'none'; };
        }
