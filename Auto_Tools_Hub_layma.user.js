...
// @version      6.85-layma
// @description  layma.net = form link4m; chỉ khác đọc #TK1/#hinh_nv + nút LẤY MÃ
// @author       You
// @match        *://*/*
// @grant        GM_setValue
// @grant        GM_getValue
...
// @all_frames   true
// @downloadURL  https://raw.githubusercontent.com/USER/REPO/main/Auto_Tools_Hub_layma.user.js
// @updateURL    https://raw.githubusercontent.com/USER/REPO/main/Auto_Tools_Hub_layma.user.js
...
(function () {
    ...
    // ===== layma what-on (bản cũ, KHÔNG tự bấm xacthucButton) =====
    let autoWhatOnEnabled = true;

    function fireTapAt(x, y) {
        const el = document.elementFromPoint(x, y) || document.body;
        try {
            const t = { identifier: 1, target: el, clientX: x, clientY: y, pageX: x + (window.scrollX || 0), pageY: y + (window.scrollY || 0), screenX: x, screenY: y, radiusX: 8, radiusY: 8, force: 1 };
    ...
    function handleLaymaWhatOnMessages() {
        ...
        const codeNow = typeof extractPromoCode === 'function' ? extractPromoCode() : '';
        if (codeNow) {
        ...
        // Có Mã Code → dừng, để auto-paste
        if (/mã\s*code\s*[:：]|ma\s*code\s*[:：]/i.test(raw)) {
            try { if (typeof stopScroll === 'function') stopScroll(); } catch (e) {}
            return false;
        }

        // Chạm màn hình — CHỈ tap giữa, KHÔNG bấm xacthucButton
        if (t.includes('chạm vào màn hình') || t.includes('cham vao man hinh') || (t.includes('chạm vào') && t.includes('màn hình'))) {
            console.log('[Auto Tools] layma: CHẠM giữa màn hình (không bấm xác thực)');
            try {
        ...
        if (t.includes('cuộn lên') || t.includes('cuon len') || t.includes('scroll up') || (t.includes('lên') && t.includes('tiếp tục'))) {
            console.log('[Auto Tools] layma: CUỘN LÊN CÙNG');
            try { if (typeof stopScroll === 'function') stopScroll(); } catch (e) {}
            try {
                window.scrollTo(0, 0);
        ...
        if (t.includes('cuộn xuống') || t.includes('cuon xuong') || t.includes('scroll down') || (t.includes('xuống') && t.includes('tiếp tục'))) {
            console.log('[Auto Tools] layma: CUỘN XUỐNG CÙNG');
            try { if (typeof stopScroll === 'function') stopScroll(); } catch (e) {}
            try {
    ...
    function watchLaymaMessages() {
        ...
        const tick = () => {
            try {
                const code = typeof extractPromoCode === 'function' ? extractPromoCode() : '';
                if (code) {
                    try { if (typeof stopScroll === 'function') stopScroll(); } catch (e) {}
                    return;
                }
                if (!autoWhatOnEnabled) return;
                if (Date.now() - lastAction < 700) return;
                if (handleLaymaWhatOnMessages()) lastAction = Date.now();
            } catch (e) {}
        };
    ...
    function isLink4mHost() {
        // Script layma: coi layma.net là "form gốc" (cùng chỗ link4m dùng isLink4mHost)
    ...
    function pickInstructionImages() {
        // layma: ưu tiên #hinh_nv, ảnh api.layma.net
        const out = [];
        const hinh = document.getElementById('hinh_nv');
        if (hinh) out.push(hinh);
        document.querySelectorAll('img').forEach(img => {
            if (out.includes(img)) return;
            const src = (img.currentSrc || img.src || '').toLowerCase();
            if (/api\.layma\.net\/media|layma\.net.*posts|layma\.net\/media/i.test(src)) out.push(img);
        });
    ...
    // layma: CHỈ text trong p#TK1 — bỏ URL nếu dính
    function cleanLaymaKeyword(raw) {
        let k = String(raw || '').replace(/\s+/g, ' ').trim();
        k = k.replace(/https?:\/\/\S+/gi, '').trim();
        k = k.replace(/www\.\S+/gi, '').trim();
        // bỏ đoạn domain dính cuối
        k = k.replace(/\s+[a-z0-9.-]+\.[a-z]{2,}(\/\S*)?$/i, '').trim();
        return k;
    }
    function scrapeKeywordDomainFromPage() {
        ...
        let domain = '';
        try {
            let el = document.querySelector('p#TK1.box-copy-code, p#TK1.no-copy, p.box-copy-code#TK1');
            if (!el) el = document.getElementById('TK1');
            if (!el) el = document.querySelector('#TK1, p.box-copy-code');
            if (el) keyword = cleanLaymaKeyword(el.textContent || el.innerText || '');
            if (!keyword) {
                for (const id of ['TK2', 'TK3']) {
                    const e2 = document.getElementById(id);
                    if (!e2) continue;
                    const v = cleanLaymaKeyword(e2.textContent || e2.innerText || e2.value || '');
                    if (v.length >= 2) { keyword = v; break; }
                }
            }
        } catch (e) {}
    ...
    async function aiFillKeywordFromImages(autoSearch) {
        ...
        try {
            ...
            keyword = cleanLaymaKeyword(keyword);
            ...
            try {
                ...
                if (pageScrape.keyword) {
                    const kEl = document.getElementById('as-keyword');
                    const dEl = document.getElementById('as-domain');
                    const ck = cleanLaymaKeyword(pageScrape.keyword);
                    if (kEl) kEl.value = ck;
                    if (dEl && pageScrape.domain) dEl.value = pageScrape.domain;
                    setSt('Scrape trang: ' + ck + ' | ' + (pageScrape.domain || ''));
                    if (autoSearch) startSmartSearch(ck, pageScrape.domain);
                    return;
                }
            } catch (e2) {}
...
(function(){
  ...
  setTimeout(function typeNext(){
    ...
    function findLayMaButton() {
        // layma: nút đổi class/id — nhận diện theo TEXT "LẤY MÃ" + style, TRÁNH nút L4M đỏ link4m
        const scored = [];
        const all = document.querySelectorAll('button, a, div[role="button"], span, div');
        for (const el of all) {
            try {
                if (el.closest && el.closest('#as-panel, #as-btn, figure, picture, .wp-block-image')) continue;
            } catch (e) {}
            const raw = (el.textContent || '').replace(/\s+/g, ' ').trim();
            const txt = raw.toUpperCase();
            // Chỉ nhận nút gần đúng "LẤY MÃ" (không L4M)
            if (!/LẤY\s*MÃ|LAY\s*MA/i.test(raw)) continue;
            if (/L4M/i.test(raw)) continue; // nút link4m minh họa
            ...
            let score = 10;
            // Ưu tiên span chữ trắng inline (user gửi)
            const st = (el.getAttribute('style') || '') + ' ' + (target.getAttribute('style') || '');
            if (/color\s*:\s*#fff|color\s*:\s*white|color\s*:\s*rgb\(255/i.test(st)) score += 25;
            let bg = '';
            try { bg = (getComputedStyle(target).backgroundColor || '').toLowerCase(); } catch (e) {}
            // Ưu tiên xanh (layma) — phạt đỏ link4m
            if (/rgb\(\s*1[6-9]\d|rgb\(\s*2[0-5]\d\s*,\s*[2-9]\d|34,\s*197|16,\s*185|green|#16a34a|#22c55e/i.test(bg + st)) score += 20;
            if (/237\s*,\s*28\s*,\s*36|#ed1c24|rgb\(\s*220/i.test(bg + st)) score -= 40;
            // Ảnh what-on.com → thường là widget đích OK nếu không nằm figure
            if (target.querySelector && target.querySelector('img[src*="what-on"]')) score += 5;
            // Visible in viewport
            if (r.top >= 0 && r.top < window.innerHeight) score += 8;
            // Cuối trang thường là nút thật
            score += Math.min(15, (r.top + (window.scrollY || 0)) / 800);
            scored.push({ el: target, score });
        }
        scored.sort((a, b) => b.score - a.score);
        if (scored.length && scored[0].score >= 10) return scored[0].el;
        return null;
    }
        ...
        panel.innerHTML = `
            <div id="panel-main">
                ...
                <div class="as-tool-btn" id="btn-link4m">
                    ...
                    <div>
                        <div style="font-weight:600">layma.net</div>
