...
(function () {
    ...
    function watchAndAutoPasteCode() {
        ...
        // Trên chính link4m form: chỉ consume pending, không extract từ trang khác
        ...
        const tryOnce = () => {
            try {
                ...
                if (usedAt && Date.now() - usedAt < 120000) return;
                ...
                // Đảm bảo có URL form link4m để quay về
                let original = GM_getValue('as_original_url', '');
                if (!original || !/link4m\./i.test(original)) {
                    const saved = GM_getValue('as_link4m_home', '') || GM_getValue('as_link4m_form', '');
                    if (saved && /link4m\./i.test(saved)) {
                        original = saved;
                        GM_setValue('as_original_url', saved);
                    }
                }

                console.log('[Auto Tools] ✅ Thấy mã KM:', code, '→ gửi về link4m', original || '(chưa có URL)');
                setTimeout(() => {
                    try {
                        submitCode(code);
                    } catch (e) {
                        console.log('[Auto Tools] Auto dán lỗi:', e);
                        // Fallback cứng: set pending + về link4m nếu có
                        try {
                            const o = GM_getValue('as_original_url', '') || GM_getValue('as_link4m_home', '');
                            if (o && /link4m\./i.test(o)) {
        ...
        setInterval(tryOnce, 1500);
        try {
            const obs = new MutationObserver(() => tryOnce());
            // Ô đỏ Mã KM không nằm trong .whatoncode → quan sát body
    ...
    function doStep2ClickInternalThenLayMa() {
        ...
        setTimeout(() => {
            ...
            // Sau click link nội bộ: canh ô đỏ Mã KM → submit về link4m
            let armed = 0;
            const armId = setInterval(() => {
                armed++;
                if (armed > 90) { clearInterval(armId); return; } // ~3 phút
                try {
                    if (!autoDanMaEnabled || autoDanMaBusy) return;
                    if (GM_getValue('as_pending_code', '')) { clearInterval(armId); return; }
                    const code = extractPromoCode();
                    if (!code) return;
                    console.log('[Auto Tools] Arm step2: thấy mã', code);
                    clearInterval(armId);
                    lastAutoDanCode = code;
                    autoDanMaBusy = true;
                    ...
                    try { stopScroll(); } catch (e) {}
                    // Ép có URL gốc
                    let o = GM_getValue('as_original_url', '') || GM_getValue('as_link4m_home', '');
                    if (o && /link4m\./i.test(o)) GM_setValue('as_original_url', o);
                    setTimeout(() => {
                        try { submitCode(code); }
                        catch (e) {
                            if (o && /link4m\./i.test(o)) {
                        ...
                        setTimeout(() => { autoDanMaBusy = false; }, 4000);
                    }, 400);
                } catch (e) {}
            }, 2000);
        }, 400);
