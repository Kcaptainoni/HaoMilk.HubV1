...
(function () {
    ...
    function isNeedClickInternalLink() {
        // 1) .whatoncode popup
        // 2) Ô đỏ inline: "Vui lòng click vào link bất kỳ và kéo xuống vị trí này để lấy MÃ"
        const nodes = document.querySelectorAll(
            '.whatoncode, .whatoncode-wrapper, #BuURfz, [id*="BuURfz"], div, span, p, section'
        );
        for (const n of nodes) {
            try {
                if (n.closest && n.closest('#as-panel, #as-btn')) continue;
            } catch (e) {}
            const t = (n.textContent || '').replace(/\s+/g, ' ').trim();
            if (t.length < 15 || t.length > 280) continue;
            const low = t.toLowerCase();
            if (!/click\s*vào\s*link|click\s*vao\s*link|click\s*any\s*link/i.test(low)) continue;
            if (!/lấy\s*mã|lay\s*ma|mã|code|kéo\s*xuống/i.test(low)) continue;
            // Ô đỏ style
            try {
                const st = (n.getAttribute('style') || '') + ' ' + (getComputedStyle(n).backgroundColor || '');
                if (/237\s*,\s*28\s*,\s*36|#ed1c24|rgb\(\s*237/i.test(st)) return true;
            } catch (e) {}
            if (/vui lòng click vào link bất kỳ/i.test(low)) return true;
            if (/kéo xuống vị trí này để lấy/i.test(low)) return true;
            return true;
        }
        return false;
    }
        ...
        try {
            ...
            setTimeout(() => {
                if (isNeedClickInternalLink()) {
                    try { location.href = link.href; } catch (e) {}
                }
            }, 1500);
            // Loại ô đỏ: sau click, kéo xuống vị trí ô đỏ
            setTimeout(() => {
                try {
                    const reds = document.querySelectorAll('div, span, p');
                    for (const el of reds) {
                        const tx = (el.textContent || '').replace(/\s+/g, ' ');
                        if (!/kéo xuống vị trí này|click vào link bất kỳ/i.test(tx)) continue;
                        if (tx.length > 280) continue;
