...
// @version      6.81-gtrafic
// @description  gtrafic standalone: AI keyword/domain, tab mới, dán mã
// @author       You
// @match        *://*/*
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    function markFlowOrigin() {
        try {
            ...
            GM_setValue('as_flow_origin', 'gtrafic');
        } catch (e) {}
    }
    try {
    ...
    function ensureUI() {
        ...
        panel.innerHTML = `
            ...
            <div id="panel-gtrafic" style="display:none">
                ...
                <div class="as-body">
                    <button class="back-btn" id="btn-back-gt">✕ Đóng</button>
            ...
            <div id="panel-layma" style="display:none">
                ...
                <div class="as-body">
                    <button class="back-btn" id="btn-back-lm">← Menu chính</button>
        ...
        btn.onclick = (e) => {
            ...
            if (open) {
                try {
                    ['panel-main','panel-layma','panel-link4m'].forEach(id => {
                        const el = document.getElementById(id); if (el) el.style.display = 'none';
                    });
                    const pg = document.getElementById('panel-gtrafic');
                    if (pg) { pg.style.display = 'block'; pg.style.setProperty('display','block','important'); }
                } catch (err) {}
        ...
        const closeGt = document.getElementById('as-close-gt');
        if (closeGt) closeGt.onclick = () => { panel.style.display = 'none'; };

        function showGtraficPanel() {
            try {
                ['panel-main','panel-layma','panel-link4m'].forEach(id => {
                    const el = document.getElementById(id); if (el) el.style.display = 'none';
                });
                ...
                if (pg) { pg.style.display = 'block'; pg.style.setProperty('display','block','important'); }
                panel.style.display = 'block';
            } catch (err) { console.log('[Auto Tools] showGtraficPanel', err); }
        }
        ['btn-open-gt','btn-gtrafic'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.onclick = (e) => { try { e.preventDefault(); e.stopPropagation(); } catch (e2) {} showGtraficPanel(); };
        });
        const backGt = document.getElementById('btn-back-gt');
        if (backGt) {
            backGt.textContent = '✕ Đóng';
            backGt.onclick = () => { panel.style.display = 'none'; };
        }
