...
(function () {
    ...
    function extractPromoCode() {
        // gtrafic extract: trade-btn code
        try {
            for (const el of document.querySelectorAll('.trade-btn-clf__content.copy-allowed, .trade-btn-clf__content')) {
    ...
    // gtrafic: scrape từ khóa trên form + title kết quả nếu có
    function scrapeKeywordDomainFromPage() {
        ...
        try {
            ...
            for (const inp of inputs) {
                if (inp.closest && inp.closest('#as-panel, #as-btn')) continue;
                const v = (inp.value || '').trim();
                const ph = (inp.placeholder || '').toLowerCase();
                const name = ((inp.name || '') + (inp.id || '')).toLowerCase();
                if (v.length >= 2 && (/key|tu|keyword|search|tìm/i.test(ph + name) || !keyword)) {
                    if (/key|tu|keyword|search|tìm/i.test(ph + name)) { keyword = v; break; }
                    if (!keyword && v.length < 80) keyword = v;
                }
            }
...
(function(){
  ...
  setTimeout(function typeNext(){
    ...
    function findLayMaButton() {
        // gtrafic: ưu tiên #trade-btn-clf / .trade-btn-clf
        try {
            const gt = document.getElementById('trade-btn-clf') ||
                document.querySelector('.trade-btn-clf-container, #avt-btn, .trade-btn-clf');
            if (gt) {
                const r = gt.getBoundingClientRect();
                if (r.width > 10 && r.height > 10) {
                    return gt.closest('#trade-btn-clf, .trade-btn-clf, .trade-btn-clf-container') || gt;
                }
            }
        } catch (e) {}
