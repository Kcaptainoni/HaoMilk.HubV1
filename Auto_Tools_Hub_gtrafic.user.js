...
// @version      6.83-gtrafic
// @description  gtrafic = form link4m; chỉ khác đọc KW/domain + nút trade-btn
// @author       You
// @match        *://*/*
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    function isBadCodeToken(t) {
        t = String(t || '').trim();
        if (!t || t.length < 4 || t.length > 20) return true;
        if (!/^[A-Za-z0-9]+$/.test(t)) return true;
        if (!/[A-Za-z]/.test(t) || !/[0-9]/.test(t)) {
            // Cho phép pure alpha nếu dài >= 5, nhưng chặn từ khóa UI
        }
        if (/^(code|mã|ma|lay|lấy|wait|copy|click|get|arrow|g)$/i.test(t)) return true;
        return false;
    }
    function extractPromoCode() {
        // gtrafic: CHỈ lấy .trade-btn-clf__content.copy-allowed đang hiện (display flex)
        try {
            const nodes = document.querySelectorAll(
                '.trade-btn-clf__content.copy-allowed, #trade-btn-clf.trade-btn-clf--show-code .trade-btn-clf__content, .trade-btn-clf--show-code .trade-btn-clf__content'
            );
            for (const el of nodes) {
                ...
                // Mã thật thường có cả chữ + số (vd bPt84Vrq)
                if (/[A-Za-z]/.test(t) && /[0-9]/.test(t) && t.length >= 5) return t;
                if (/^[A-Za-z0-9]{6,16}$/.test(t) && /[A-Za-z]/.test(t)) return t;
            }
        } catch (e) {}
        // Không fallback regex "code:" trên gtrafic (dễ bắt nhầm chữ Code)
        // 1) Div đỏ what-on (trang đích)
        const candidates = [];
        try {
    ...
    // gtrafic: ĐÚNG span font-mono font-bold notranslate (vd sunwi)
    function scrapeKeywordDomainFromPage() {
        ...
        try {
            const sels = [
                ...
                'span.font-mono.font-bold.text-lg.notranslate',
                'span.font-mono.font-bold.notranslate',
                'span.notranslate.font-mono.font-bold'
            ];
            for (const sel of sels) {
                const el = document.querySelector(sel);
                if (!el) continue;
                const v = (el.textContent || el.innerText || '').replace(/\s+/g, ' ').trim();
                if (v.length >= 2 && v.length < 80 && !/^https?:/i.test(v)) {
                    keyword = v;
                    break;
                }
            }
            if (!keyword) {
                document.querySelectorAll('span.font-mono, span.notranslate').forEach(el => {
                    if (keyword) return;
                    if (el.closest && el.closest('#as-panel, #as-btn, #trade-btn-clf')) return;
                    const v = (el.textContent || '').replace(/\s+/g, ' ').trim();
                    const cls = (el.className || '').toString();
                    if (/font-mono|font-bold/i.test(cls) && v.length >= 2 && v.length <= 40 && /^[\w.-]+$/.test(v)) {
                        keyword = v;
                    }
                });
            }
